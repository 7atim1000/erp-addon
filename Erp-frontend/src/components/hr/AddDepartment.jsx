...
import { addDepartment } from '../../https';

const AddDepartment = ({setIsDepartmentAddModal}) => {

    const userData = useSelector((state) => state.user);
    const userId = userData ? userData._id : "";

    const handleClose = () => {
        setIsDepartmentAddModal(false)
    };

    const [formData, setFormData] = useState({
        departmentName: "", departmentNo: `${Date.now()}`, user: userId
    });
    ...
    const departmentMutation = useMutation({
        mutationFn: (reqData) => addDepartment(reqData),
        onSuccess: (res) => {
        ...
        <div className='fixed inset-0 bg-opacity-50 flex items-center justify-center shadow-lg/10 z-50' style={{ backgroundColor: 'rgba(20, 10, 10, 0.4)' }}>
            ...
            >
                ...
                <div className="flex justify-between items-center mb-2 shadow-xl p-2">
                    <h2 className='text-[#1a1a1a] text-md font-bold'>Add Department</h2>
                ...
                <form className='mt-3 space-y-6' onSubmit={handleSubmit}>
                    <div className='mt-5'>
                        ...
                        <label className='w-[25%] text-[#1a1a1a] block mb-2 mt-3 text-xs font-normal'>Department Name :</label>
